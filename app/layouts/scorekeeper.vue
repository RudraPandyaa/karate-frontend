...
const menu = [
  ...
  { label: 'My Matches', to: '/scorekeeper/matches', icon: Swords },
  { label: 'Results', to: '/scorekeeper/results', icon: Trophy },
  { label: 'Profile', to: '/scorekeeper/profile', icon: User },
]
  ...
  <div class="flex h-screen overflow-hidden bg-canvas text-foreground">
    <aside class="hidden h-full w-64 shrink-0 flex-col border-r border-line bg-panel lg:flex">
      ...
      <div class="shrink-0 border-t border-line p-4">
        <div class="mb-3 flex items-center gap-3">
          ...
          <div class="min-w-0">
            <p class="truncate text-sm font-medium">{{ user?.name || 'Scorekeeper' }}</p>
            <p class="text-xs text-muted">Scorekeeper</p>
          </div>
        </div>
        <button
    ...
    >
      <div class="flex h-14 items-center justify-between border-b border-line px-4">
        <p class="font-bold">Scorekeeper</p>
        <button class="rounded-lg p-2 text-muted" @click="mobileOpen = false">
          <X class="h-5 w-5" />
        </button>
      </div>
