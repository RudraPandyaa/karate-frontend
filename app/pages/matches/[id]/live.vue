...
// Fires on mount (covers landing on an already-finished match) and
// whenever status transitions to COMPLETED live.
watch(
  () => match.value?.status,
  (status) => {
    console.log('[winner watch] status:', status, 'winnerData:', winnerData.value)
