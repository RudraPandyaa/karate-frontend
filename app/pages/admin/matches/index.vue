...
<template>
  <div class="min-h-screen bg-zinc-950 px-6 py-10 text-white">
    <div class="max-w-5xl mx-auto space-y-10">
      <h1 class="text-3xl font-bold">Manage Matches</h1>

      <!-- Form -->
      <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
        <h2 class="text-lg font-semibold">{{ editingId ? 'Edit Match' : 'Create Match' }}</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <select v-model="form.categoryId" class="w-full bg-zinc-800 rounded-lg px-3 py-2">
          ...
          <div>
            <label class="text-xs text-white/50 block mb-1">Tatami</label>
            <select v-model="form.tatamiId" class="w-full bg-zinc-800 rounded-lg px-3 py-2" :disabled="!form.categoryId">
          ...
          <div>
            <label class="text-xs text-white/50 block mb-1">Round</label>
            <input v-model="form.round" placeholder="e.g. Quarter Final" class="w-full bg-zinc-800 rounded-lg px-3 py-2" />
          </div>

          <div>
            <label class="text-xs text-white/50 block mb-1">Status</label>
            <select v-model="form.status" class="w-full bg-zinc-800 rounded-lg px-3 py-2">
          ...
          <div>
            <label class="text-xs text-white/50 block mb-1">Red Corner Athlete</label>
            <select v-model="form.redAthleteId" class="w-full bg-zinc-800 rounded-lg px-3 py-2">
          ...
          <div>
            <label class="text-xs text-white/50 block mb-1">Blue Corner Athlete</label>
            <select v-model="form.blueAthleteId" class="w-full bg-zinc-800 rounded-lg px-3 py-2">
              <option value="">TBD</option>
              <option v-for="a in athletes" :key="a.id" :value="a.id">{{ a.name }}</option>
            </select>
          </div>
        ...
        <div class="flex gap-3 pt-2">
          <button
            class="px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 font-semibold disabled:opacity-40"
            :disabled="saving"
            @click="submit"
          >
          ...
          <button
            v-if="editingId"
            class="px-5 py-2 rounded-lg bg-zinc-700 hover:bg-zinc-600 font-semibold"
            @click="resetForm"
          >
            Cancel
          </button>
      ...
      <!-- List -->
      <div>
        <h2 class="text-lg font-semibold mb-4">Existing Matches</h2>

        <div v-if="matchesPending" class="text-white/60">Loading...</div>
        ...
        <div v-else class="space-y-2">
          <div
            v-for="m in matches"
            :key="m.id"
            class="flex items-center justify-between bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-3"
          >
            <div>
              <p class="font-medium">
            ...
            <div class="flex gap-2">
              <NuxtLink
                ...
                class="px-3 py-1.5 rounded-lg bg-blue-700 hover:bg-blue-600 text-sm"
              >
                Score
              </NuxtLink>

              <button
                class="px-3 py-1.5 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-sm"
                @click="startEdit(m)"
              >
                Edit
              </button>
              ...
              <button
                class="px-3 py-1.5 rounded-lg bg-red-800 hover:bg-red-700 text-sm"
